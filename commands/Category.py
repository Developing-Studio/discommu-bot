...
class Command(BaseCommand):
    ...
    @category.command(name = '추가', usage = '추가 [이름] [설명]', aliases = ['add', 'new'], help = '카테고리를 추가합니다')
    async def add_category(self, ctx, name: str, *, description: str):
        if len(list(self.bot.categoryCollection.find({'name': name}))):
        ...
        self.bot.categoryCollection.insert({
    ...
    @category.command(name = '삭제', usage = '삭제 [이름]', aliases = ['del', 'delete', 'remove'], help = '카테고리를 삭제합니다')
    async def del_category(self, ctx, name: str):
        if not len(list(self.bot.categoryCollection.find({'name': name}))):
            await ctx.send(embed = Embed(title = '그런 이름을 가진 카테고리가 없습니다', color = Color.red()))
            return

        category = self.bot.categoryCollection.find_one({'name': name})
        ...
        self.bot.categoryCollection.remove({
            'authorID': str(ctx.author.id),
            'name': name
        })
        await msg.edit(embed = Embed(title = '카테고리를 삭제했습니다', color = Color.green()))

    @category.command(name = '수정', usage = '수정 [이름]', aliases = ['edit', 'update'], help = '카테고리의 설명을 수정합니다')
    async def edit_category(self, ctx, name: str):
        ...
        self.bot.categoryCollection.update_one({
            'authorID': str(ctx.author.id),
            'name': name
        },
        {
    ...
    async def info_category(self, ctx, name: str):
        ...
        await ctx.send(embed = Embed(
            ...
            description = f'**설명:** `{category["description"]}`\n**글 수:** `{len(list(self.bot.postCollection.find({"category": name})))}`',
            color = Color.green()
        ))
    ...
    async def list_category(self, ctx):
        ...
        for categories in divide(list(self.bot.categoryCollection.find()), 10):
            embed = Embed(
                title = '카테고리 목록',
                color = Color.green()
            )
